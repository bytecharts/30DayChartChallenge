...
top_growth <- facet_data %>%
  slice_max(order_by = growth, n = 1, with_ties = FALSE)
top_share <- facet_data %>%
  slice_max(order_by = last_share, n = 1, with_ties = FALSE)

plot_subtitle <- paste0(
  "Since 2000, China moved above <span style='color:",
  night_owlish_cat[9],
  ";'><b>50% share of global production in ",
  n_above_50,
  " of ",
  n_total,
  " sectors</b></span>.",
...
share_plot <- ggplot(
  ...
  geom_point(
    data = facet_data,
    aes(x = first_year, y = first_share, color = growth),
    alpha = 0.001,
    show.legend = TRUE
  ) +
  geom_hline(
  ...
  scale_fill_gradientn(
    colors = growth_band_colors_alpha,
  ...
  scale_x_continuous(
    breaks = seq(
      floor(min(trend_df$year) / 5) * 5,
      ceiling(max(trend_df$year) / 5) * 5,
      by = 5
    ),
