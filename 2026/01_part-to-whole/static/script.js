...
const draw = (data, colorMap, ballSymbol) => {
  ...
  const stageHeight = chartEl.clientHeight || Math.max(420, width * 0.6);
  const plotOffsetY = 12;
  const renderScale = 4; // try 2 or 3
  const plotHeight = Math.max(620, stageHeight - plotOffsetY - 90);
  ...
  const root = d3
    ...
    .sum((d) => Math.pow(d.Valuation_USD_Mn, exponent))
    .sort((a, b) => b.value - a.value);

  d3.pack().size([width, plotHeight]).padding(14)(root);

  const plotGroup = svg
    .append("g")
    .attr("class", "plot-group")
    .attr("transform", `translate(0,${plotOffsetY})`);

  const centerX = width / 2;
  const centerY = plotHeight / 2;
  const startX = width * 0.85;
  const startY = -60;
  const spreadX = width < 340 ? 13.6 : 1;
  const getX = (d) => centerX + (d.x - centerX) * spreadX;
  const getY = (d) => d.y;
...
Promise.all([
  ...
  const total = d3.sum(data, (d) => d.Valuation_USD_Mn);
  totalValueEl.textContent = `$${formatNumber(total)}M`;
